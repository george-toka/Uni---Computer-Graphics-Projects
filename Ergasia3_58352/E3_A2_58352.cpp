...
glm::vec2 getResolution(GLFWwindow* givenWindow,int givenWidth, int givenHeight)
{
...
#pragma endregion glfw, glew, window, depth testing, ImGui.
...
#pragma endregion cube vertices, cube positions.
	...
	Shader shaderProgram("res/Shaders_58352/E3_A2_VertexShader.txt", "res/Shaders_58352/E3_A2_FragmentShader.txt");
	Shader ligthSourceShader("res/Shaders/LightSourceVertexShader.glsl", "res/Shaders/LightSourceFragmentShader.glsl");

	unsigned int brickwallTexture = GenerateTexture("res/Images/Water.jpg");
	unsigned int brickwallNormalTexture = GenerateTexture("res/Images/Water_normal.jpg");

#pragma region Rendering Loop

	//Normal Mapping Toggle Flag 
	bool NormalMappingSwitch = true;

	//Flags to use key input as toggle
	bool KEY_PRESSED1 = false;
	bool KEY_RELEASED1 = true;

	bool KEY_PRESSED2 = false;
	bool KEY_RELEASED2 = true;

	glm::vec2 resolution = { 800,600 };

	bool rippleEffect = true;
...
#pragma region Quad
		...
		shaderProgram.setVec2("resolution", resolution);
		shaderProgram.setFloat("time", time);
		shaderProgram.setBool("rippleEffect", rippleEffect);
		...
		shaderProgram.setBool("FragNormalMappingSwitch", NormalMappingSwitch);
		shaderProgram.setBool("VertNormalMappingSwitch", NormalMappingSwitch);
		...
		glm::mat4 model = glm::translate(identityMatrix, cubePositions);
...
#pragma region ImGui window and tools
		...
		ImGui::Checkbox("Normal Mapping Toggle", &NormalMappingSwitch);
		ImGui::Text("Use KEY_1 to tick/untick");

		ImGui::Checkbox("Waves - Toggle", &rippleEffect);
		ImGui::Text("Use KEY_2 to tick/untick");

		//Toggle the checkbox with key input 
		if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS && KEY_PRESSED1 != KEY_RELEASED1)
		{
			NormalMappingSwitch = !NormalMappingSwitch;
			KEY_PRESSED1 = !KEY_PRESSED1;
		}

		if (glfwGetKey(window, GLFW_KEY_1) == GLFW_RELEASE && KEY_PRESSED1 == KEY_RELEASED1)
			KEY_PRESSED1 = !KEY_PRESSED1;

		//2nd Checkbox
		if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS && KEY_PRESSED2 != KEY_RELEASED2)
		{
			rippleEffect = !rippleEffect;
			KEY_PRESSED2 = !KEY_PRESSED2;
		}

		if (glfwGetKey(window, GLFW_KEY_2) == GLFW_RELEASE && KEY_PRESSED2 == KEY_RELEASED2)
			KEY_PRESSED2 = !KEY_PRESSED2;
